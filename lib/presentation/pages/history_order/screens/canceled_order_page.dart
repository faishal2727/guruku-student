...
class CanceledOrderPage extends StatefulWidget {
  const CanceledOrderPage({super.key});

  @override
  State<CanceledOrderPage> createState() => _CanceledOrderPageState();
}

class _CanceledOrderPageState extends State<CanceledOrderPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      context.read<OrderCancelBloc>().add(OnOrderCanceledEvent());
    });
  }
      ...
      body: RefreshIndicator(
        ...
        child: BlocBuilder<OrderCancelBloc, OrderCancelState>(
          builder: (context, state) {
            if (state is OrderCanceledLoading) {
              return Center(
                child: Lottie.asset(
            ...
            if (state is OrderCanceledHasData) {
            ...
            } else if (state is OrderCanceledError) {
              return ErrorSection(
                isLoading: _isLoading,
                onPressed: _retry,
                message: state.message,
              );
            } else if (state is OrderCanceledEmpty) {
